...
export function createCommands(world: World): Command[] {
	...
	command(['collect'], '/collect <kind> <amount> - grant/remove counters (admin only)', 'admin', ({ }, client, message) => {
		...
		if ([ 'gifts', 'eggs', 'clovers', 'candies' ].includes(kind)) {
			...
			const kindNames: any = { gifts: 'подарков', eggs: 'яиц', clovers: 'клевер(ов)', candies: 'конфет' };
			saySystem(client, `${amount >= 0 ? 'Вы выдали' : 'Вы удалили'} ${Math.abs(amount)} ${kindNames[kind]}`);
		} else {
			// toy(s)
			...
			if (m === 0) {
				saySystem(client, 'Неверный параметр для игрушек');
				return;
			}
			...
			saySystem(client, `${amount >= 0 ? 'Вы выдали' : 'Вы удалили'} игрушки 1-${m}`);
		}
	}),
